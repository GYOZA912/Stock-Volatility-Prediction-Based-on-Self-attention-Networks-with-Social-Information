...
if __name__ == '__main__':
    ...
    train_x_g = np.zeros((0, hp.maxlen, 8))
    train_y_g = np.zeros((0, 2))
    test_x_g = np.zeros((0, hp.maxlen, 8))
    ...
    if train:
        ...
        stock_price_model_path = hp.model3_path + 'general' + "/"
        ...
        log_train.write("开始训练 {}".format('general'))
        print("开始训练 {}".format('general'))
        for i in range(hp.price_num_epochs):
            # 训练数据生成器
            batches_train = price_batch(train_x_g, train_y_g)
            # 随模型进行训练 降低学习率
        ...
        batches_test = price_batch(test_x_g, test_y_g)
