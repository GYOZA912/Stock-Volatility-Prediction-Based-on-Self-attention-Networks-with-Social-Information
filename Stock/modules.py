...
def embedding(inputs, 
    ...
    :param num_units: An int. Number of embedding hidden units.
    :param zero_pad: A boolean. If True, all the values of the first row (id 0) should be constant zeros.
    :param scale: A boolean. If True. the outputs is multiplied by sqrt num_units.
    :param scope: Optional scope for `variable_scope`.
    :param reuse:  Boolean, whether to reuse the weights of a previous layer by the same name.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if zero_pad:
            # 将embedding的第一行所有元素用0替换，即pad对应的向量0初始化
...
def positional_encoding(inputs,
    ...
    :param inputs:  A 2d Tensor with shape of (N, T).
    :param num_units: Output dimensionality
...
def multihead_attention(queries, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Key Masking  将key中每个词对应的token变为0或者1
        # 想让那些key值的unit为0的key对应的attention score极小，这样在加权计算value的时候相当于对结果不造成影响
        # 其实主要是去除pad的影响，因为之前pad初始化为0
        key_masks = tf.sign(tf.abs(tf.reduce_sum(keys, axis=-1)))  # (N, T_k)
        key_masks = tf.tile(key_masks, [num_heads, 1])  # (h*N, T_k)
        key_masks = tf.tile(tf.expand_dims(key_masks, 1), [1, tf.shape(queries)[1], 1])  # (h*N, T_q, T_k)

        # 用很小的值来替换对应位置的output值，目的不太懂？
        paddings = tf.ones_like(outputs)*(-2**32+1)
        outputs = tf.where(tf.equal(key_masks, 0), paddings, outputs)  # (h*N, T_q, T_k)

        # Causality = Future blinding
        # 用于dec的self_attention阶段，当只输入一个词的时候，这个词只能和自身做attention，目的是与生成时候的逻辑匹配
        if causality:
            # 为啥有这个操作，与dec的输入有关，每次都是一句话，即使只有一个<GO>的时候，因此不能对<PAD>部分进行attention
            diag_vals = tf.ones_like(outputs[0, :, :])  # (T_q, T_k)
            '''
            将矩阵变成下三角矩阵:
            
            假设有一个(T_q=5,T_k=5)的矩阵，
            for i in range(T_q=5):
                for j in range(T_k=5):
                    if j > i :
                        A[i][j]=0
            ...
            output是Q和K的乘积，这样做的意义在于，对于第一行来说，每个值代表的是query对应词向量与key对应词向量的点乘
            '''
...
def feedforward(inputs, 
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # 卷积操作，"filters": num_units[0]代表的是输出的维度
